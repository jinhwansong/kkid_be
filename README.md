...
**기술 스택**: React, NestJS, TypeScript, Mux, Redis, PostgreSQL, TypeORM

**담당 영역**:
- ✅ 프론트엔드 (영상 업로드 UI, 상태 관리, 진행률 표시)
- ✅ 백엔드 API 설계 (NestJS — 영상 업로드, 메타데이터, 댓글, 조회수)
- ✅ Jammit 도메인 API (`/auth`, `/user`, `/gatherings`, `/review` 등 — 로그인·유저·모임·참가·리뷰). 상세는 저장소 루트 `md/project.md` 참고. DB 시드: `db/seed/`, `npm run seed:gathering` 등
...
- Redis (조회수 중복 방지, 이메일 인증 코드)
- Mux API (영상 스트리밍)
- JWT 로컬 검증 (`JWT_SECRET_KEY`), Jammit: `src/app/app.module.ts`에 모듈 등록 + `src/jammit-*` 도메인 폴더
